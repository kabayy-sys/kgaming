...
import { Home, LayoutDashboard, CalendarCheck, Clock, Monitor } from 'lucide-react';

const navItems = [
  { href: '/', label: 'Home', icon: Home, authRequired: false },
  { href: '/dashboard', label: 'Dashboard', icon: LayoutDashboard, authRequired: true },
  { href: '/devices', label: 'Devices', icon: Monitor, authRequired: true },
...
export function BottomNav() {
  ...
  return (
    <nav className="bottom-nav pb-safe-bottom">
      <div className="mx-auto flex max-w-app-lg items-center justify-between px-2 py-1">
        {navItems.map((item) => {
        ...
        {!isAuthenticated && (
          <button
            ...
            className="flex flex-col items-center gap-0.5 rounded-xl px-3 py-2 text-gaming-500 hover:text-gaming-300 transition-colors"
          >
